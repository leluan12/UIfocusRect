	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				//-------------------add----------------------
               	//color.a*=(distance(IN.worldPosition.xy,_Center.xy) > _Slider);
                float2 A = _Center.xy + float2(0,_SliderY) + (float2(-_SliderX,0))+float2(-10,10);
                float2 B = _Center.xy + float2(0,-_SliderY) +(float2(-_SliderX,0))+float2(-10,-10);
                float2 C = _Center.xy + float2(_SliderX,0) +  float2(0,-_SliderY)+float2(10,-10);
                float2 M = IN.worldPosition.xy;
                float2 BC = C-B;
                float2 AB = B-A;
                float2 BM = M-B;
